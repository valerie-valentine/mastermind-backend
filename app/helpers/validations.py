...
def ensure_valid_game_data(request_data):
    if "lives" in request_data:
        if not isinstance(request_data["lives"], int) or int(request_data["lives"]) > 20 or int(request_data["lives"]) < 3:
            abort(make_response(
                {"details": "Invalid Choice: Please enter a numerical value for lives between 3 and 20"}, 400))
    if "num_min" in request_data:
        if not isinstance(request_data["num_min"], int) or request_data["num_min"] < 0:
            abort(make_response(
                {"details": "Invalid Choice: Please enter a numerical value for num_min greater than or equal to 0 and less than 9"}, 400))
    ...
    if "difficulty_level" in request_data:
        if not request_data["difficulty_level"] in [4, 6, 8]:
            abort(make_response(
                {"details": "Invalid choice: Please enter a valid level: easy, medium, hard"}, 400))
...
def ensure_valid_client_login(request_data):
    ...
    if not isinstance(email, str) or not isinstance(password, str):
        abort(make_response(
            {'details': 'Failed login. Please enter valid email or password'}, 400))
